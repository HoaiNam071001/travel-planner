...
export interface ParsedMapsPlace {
  name: string;
  lat: number;
  lng: number;
}
...
export function parseGoogleMapsUrl(url: string): ParsedMapsPlace | null {
  const placeMatch = url.match(/\/place\/([^/@]+)/);
  const name = placeMatch?.[1] ? decodeURIComponent(placeMatch[1].replace(/\+/g, " ")) : "";
  ...
  if (!coordMatch?.[1] || !coordMatch[2]) return null;
