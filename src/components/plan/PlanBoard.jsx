...
export default function PlanBoard({
  ...
  function handleDragEnd({ active, over }) {
    ...
    const toLane = overData?.type === "item" ? overData.laneId : overData?.laneId ?? over.id;
    ...
    if (overData?.type === "item" && toLane !== LIBRARY_LANE) {
      ...
      onMoveItem(active.id, toLane, toIndex === -1 ? null : toIndex);
      return;
    }
    onMoveItem(active.id, toLane, null);
  }
    ...
    <div className="animate-fade-up">
      ...
      {!hasLocations && (
        <p className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-xs text-amber-800">
          Chưa có địa điểm nào — hoạt động tạo ở đây sẽ chưa gắn địa điểm. Thêm địa điểm ở trang
          Địa điểm rồi sửa lại hoạt động khi cần.
        </p>
      )}
...
function AddUnitLane({ freeUnits, onAddUnit, onQuickCreateUnit }) {
